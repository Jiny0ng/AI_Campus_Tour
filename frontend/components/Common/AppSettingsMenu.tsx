...
export function AppSettingsMenu({ className }: AppSettingsMenuProps) {
  ...
  return (
    <div ref={containerRef} className={cn("relative flex flex-col items-end", className)}>
      <button
        ...
        className="grid size-11 place-items-center rounded-full bg-surface/95 text-ink shadow-card backdrop-blur-sm transition active:scale-95"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-[calc(100%+8px)] w-44 overflow-hidden rounded-2xl border border-line bg-surface/95 p-2 shadow-sheet backdrop-blur-md"
          >
            <button
              type="button"
