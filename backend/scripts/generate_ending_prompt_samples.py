...
def sample_texts() -> dict[str, str]:
    return {
        ...
        "tour_03_university_headquarters": (
            ...
            "표범상에 담긴 전북대의 기상을 한번 떠올려 보세요..."
        ),
        "docent_leopard_statue": (
            "지금 보고 계신 조형물은 전북대학교를 상징하는 표범상입니다. 학교는 표범이 "
            "지닌 지혜와 용감함, 과감하게 전진하는 모습을 구성원이 지향하는 품성과 "
            "기상으로 여기고 있는데요. 이 상징동물은 1981년 약 6천 명의 "
...
def main() -> int:
    ...
    preset = replace(
        BUBBLY_DOCENT,
        id="bubbly-proud-senior-story-sample-v3",
    ...
    for position, (entity_id, text) in enumerate(sample_texts().items(), start=1):
        ...
        version = "story-prompt-sample-v3"
